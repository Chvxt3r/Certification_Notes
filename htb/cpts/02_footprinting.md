...
```
* Example Reverse Name Resolution Zone File
```bash
...
```
* DIG - AXFR Internal Zone Transfer(Subdomain)
```bash
...
```
* Using VRFY to enumerate users
> Depending on how SMTP is configured, this may verify users that don't exist, making this useless.  
```bash
telnet 10.129.14.128 25
...
```
> We can also check for open relays using the `smtp-open-relay` script.  
```bash
...
```
- OpenSSL-TLS Encrypted POP3 (Interactive)
```bash
...
```
- OpenSSL-TLS Encrypted IMAP (Interactive)
```bash
...
- Connection possible by Windows Auth and by default encryption is not enforced.
### Dangerous Settings
> Alot of opportunity for misconfiguration, keep an eye out for items listed below.   
- MSSQL clients not using encryption
- Self-signed certificates when encryption is used
- Use of named pipes
- Weak & default `sa` credentials. Admin's often forget to disable this account
### Footprinting MSSQL
#### Nmap
```bash
...
```
## IPMI
Intelligent Platform Management Interface `IPMI` is a set of standardized specifications for hardware-based host management. It provides sysadmins the ability to modify settings, install OS's, update firmware, and other management tasks without relying on the OS. It normally operates on separate hardware than the main system CPU, memory, and storage by using a baseboard management controller `bmc`. They go by many common names depending on the manufacture (Dell's `DRAC`, HP `ILO`, and Supermicro `IPMI`) **Gaining access to the IPMI is basically the same as having physical access to the server**
### Footprinting
- with nmap
```
sudo nmap -sU --script ipmi-version -p 623 [ip/hostname]
```
- with metasploit
```
...
|HP iLo|Administrator|randomized 8-character string consisting of numbers and uppercase letters(Usually attached to a brown tag on the system if you can get physical access|
